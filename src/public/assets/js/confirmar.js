window.onload = function () {
  ...
  const formaPagamento =
    sessionStorage.getItem("formaPagamento") || "Não informado";
  const pagamento = sessionStorage.getItem("pagamento") || "0,00 R$";
  const endereco =
    sessionStorage.getItem("endereco") || "Endereço não informado";
...
function confirmarCompra() {
  ...
  fetch("http://localhost:3000/pedido")
    ...
    .then((pedido) => {
      ...
      let ultimoId =
        pedido.length > 0 ? Number(pedido[pedido.length - 1].id) : 0;
